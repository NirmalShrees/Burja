      ...
      >
        ...
        <div className="flex items-center gap-1.5 shrink-0">
          {phase === 'waiting' ? (
            ...
            <div className="h-[25px] flex items-center gap-1 px-2 rounded-lg bg-emerald-500/15 border border-emerald-400/30 text-emerald-300 text-[10.5px] font-bold shrink-0 font-mono">
              <span>Payouts ({Math.max(0, bettingTimer)}s)</span>
            </div>
          )}
        </div>
            ...
            {visiblePlayers.map((player) => {
              ...
              return (
                <div
                  ...
                  title={`${player.username}${isHost ? ' (Table Leader 👑)' : ''}${isDisconnected ? ' (Reconnecting...)' : ''}: ${player.isReady ? 'Ready' : 'Betting'}${isOwner && player.sessionStats?.winRate !== undefined ? ` • Win Rate: ${player.sessionStats.winRate}%` : ''}`}
                >
                  <div
            ...
            <div className="space-y-2 overflow-y-auto overflow-x-hidden pr-1 scrollbar-thin min-h-0 flex-1">
              {uniquePlayers.map((p) => {
                ...
                return (
                  <div
                    key={p.id}
                    className={`flex items-center justify-between gap-2.5 p-2.5 rounded-xl border overflow-hidden ${
                      isMe
                  ...
                  >
                    <div className="flex items-center gap-2.5 min-w-0 flex-1">
                      <div className="w-8 h-8 rounded-full bg-slate-800 border border-slate-700 flex items-center justify-center text-sm shrink-0 overflow-hidden relative">
