...
export class GameEngine {
  ...
  private startRoomLoop(roomId: string) {
    ...
    const interval = setInterval(() => {
      ...
      if (room.phase === 'betting') {
        ...
        // Advance if all active players clicked Next Round, or when the payout timer reaches 0
        if (allReady || room.timer <= 0) {
          this.startNewRound(room);
        }
      }
    }, 1000);
  ...
  public voteNextRound(roomId: string, userId: string): { success: boolean; allReady: boolean; votes: string[]; message?: string } {
    ...
    const playerIds = Object.keys(room.players);
    const allReady = playerIds.length > 0 && playerIds.every((id) => room.nextRoundVotes!.includes(id));
