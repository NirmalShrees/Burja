...
export interface VoicePeer {
  ...
  volumeLevel: number; // Live 0 to 100 for audio visualizer
}
...
class VoiceService {
  ...
  // Web Audio buffer fallback for environments where WebRTC P2P fails
  ...
  private setupSocketListeners() {
    ...
    this.socket.on('voice:user_speaking', (data: { userId: string; isSpeaking: boolean; volume: number }) => {
      if (this.state.peers[data.userId]) {
        this.state.peers[data.userId].isSpeaking = data.isSpeaking;
        this.state.peers[data.userId].volumeLevel = data.volume;
        this.notify();
      }
    });
  ...
  /**
   * Starts Voice Activity Detection (RMS energy calculation)
   */
  private startVAD(stream: MediaStream) {
    try {
      ...
      const checkVolume = () => {
        ...
        this.state.localVolumeLevel = normalized;

        const isSpeakingNow = !this.state.isMuted && normalized > 12;

        if (isSpeakingNow !== this.lastSpeakingState) {
          clearTimeout(this.speakingDebounceTimer);
          if (isSpeakingNow) {
            this.state.isSpeaking = true;
            this.lastSpeakingState = true;
            this.notify();
            this.broadcastSpeakingState(true, normalized);
          } else {
